<template>
  <m-card title="Pods" padded>
    ...
    <t-table v-if="pods.length > 0" :data="pods" :columns="columns" cell-empty-content="-" row-key="id" :stripe="true" tableLayout="auto" size="small">
      <template #custom_status="{ row }">
        <t-tooltip v-if="row.status.containerStatuses[0].state.running" :content="row.status.containerStatuses[0].state.running.startedAt" theme="light">
          <t-tag theme="success">Running</t-tag>
        </t-tooltip>
        <t-tooltip v-else-if="row.status.containerStatuses[0].state.waiting" :content="row.status.containerStatuses[0].state.waiting.message" theme="light">
          <t-tag theme="warning">
            <template #icon>
              <ErrorCircleIcon />
            </template>
        ...
        <t-tooltip
          v-else-if="row.status.containerStatuses[0].state.terminated"
          :content="row.status.containerStatuses[0].state.terminated.message"
          theme="light"
        >
          <t-tag theme="danger">
            <template #icon>
  ...
  <m-card title="Logs" padded v-if="pods.length > 0">
    <template #action>
      <t-space align="end">
        ...
        <t-button size="small" @click="autoRefreshLogs" variant="outline" :theme="autoRefreshing ? 'default' : 'primary'">
...
import { GetReleasePod } from "@/api/my";
...
const columns = ref([
  ...
  {
    colKey: "status.containerStatuses[0].image",
    title: "ImageVersion",
    cell: (_: any, { row }: any) => row.status?.containerStatuses[0]?.image?.split(":").slice(-1),
  },
...
const requestLogs = () => {
  ...
  GetMyReleasePodLogs(props.releaseId, { pod_name: currentLogsTab.value, tail_lines: tailLines.value })
    .then((resp) => {
      if (resp.code === 0) {
